...
import { getOrganizationByDomain, getOrganizationWithUserMetrics } from './organizations'
...
export async function getUsersByOrganizationId(organizationId: number, where?: string) {
  return await db
    .select({
      id: users.id,
...
export async function getUserNameAndIdByOrganizationId(organizationId: number, where?: string) {
  return await db
    .select()
    .from(users)
    .where(
...
export async function createUser(
  ...
  const orgWithMetrics = await getOrganizationWithUserMetrics(organization?.id ?? -1)
  
  const role = orgWithMetrics && orgWithMetrics.metrics.users === 0 ? 'admin' : 'user'

  return await db
    .insert(users)
    .values({
      email, password: hash, firstName, lastName, organizationId: orgWithMetrics?.id, role
    })
}
...
export async function setUserOrganization(userId: number, organizationId: number | null) {
