...
export default async function Page() {
  ...
  if (!id)
    return (
      <NotFoundError>
        <p className="text-center text-muted-foreground">Error al traer los datos del usuario.</p>
      </NotFoundError>
    )
  ...
  if (!organizationId)
    return (
      <NotFoundError>
        <p className="text-center text-muted-foreground">Este usuario no esta asignado a ninguna organización.</p>
      </NotFoundError>
    )
  ...
  const sortOrders = <T extends Order>(orders: T[]) =>
    orders.sort(
      (a, b) =>
        (a.shippingDate?.getTime() ?? Date.now()) - (b.shippingDate?.getTime() ?? Date.now())
    )
        ...
        <div className="max-w-6xl mx-auto grid gap-8">
          ...
          <div className="grid gap-4">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-bold">Pedidos que esperan tu respuesta</h2>
              <Link
                href="/app/orders"
            ...
            {orders?.map((order, index) => (
              <div key={index} className="flex flex-row bg-background text-sm p-2 relative">
